...
namespace Cifrados.Cifrados
{
    public class Cesar
    {
        public void TodoCesar(FileStream ArchivoImportado, int Key, string opcion, string textoparacomprimir)
        {
            var nombreArchivo = Path.GetFileNameWithoutExtension(ArchivoImportado.Name);
            ArchivoImportado.Close();
            
                var txtInicio = textoparacomprimir;
                var txtFinal = string.Empty;
                if (opcion == "Cifrado")
                {
                    txtFinal = CifrarCesar(txtInicio, Key);
                using (var writeStream = new FileStream(("Mis Cifrados/CIFRADO_" + opcion + "_" + nombreArchivo + ".csr"), FileMode.OpenOrCreate))
                {
                ...
                using (var writeStream = new FileStream(("Mis Cifrados/DECIFRADO_" + opcion + "_" + nombreArchivo + ".txt"), FileMode.OpenOrCreate))
                {
                    using (var writer = new BinaryWriter(writeStream))
                    {
                        writer.Write(txtFinal);
